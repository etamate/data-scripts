...
Gridded Population of the World, Version 3 (GPWv3): National Identifier Grid
...
This archive contains the national identifier grid in ASCII format. The raster 
data are at 2.5 arc-minutes resolution and contain the following data:

glbndsg		Grid identifying cells that belong to a country/state by a unique
		numeric value. 
bndsg.dbf	A lookup table that relates the grid values to state/country names 
		and codes.

Fields in the bndsg.dbf file:

ISO3V10		3-letter country/state code
UNSDCODE	Numeric country/state code from the UN Statistical Division. Codes
		with values greater than 1,000 have been created for regions that do
		not have a UNSD code.
COUNTRYENGLISH	Country/state names.
